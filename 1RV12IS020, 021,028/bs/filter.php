...
font-size: 16px;"><a href="logout.php" class="btn btn-danger square-btn-adjust">Logout</a> </div>
        ...
        <div id="page-wrapper" >
			...
			if($dept!= '' && $sem!='')
  				...
  				$sql = "SELECT `averify`,`susn`,`sfname`,`slname`,`exam_type`,`month`,`sem`,`challan_no`,`amount` FROM `exam_reg` WHERE `checks` ='$s'  AND `susn` LIKE '1RV__$dept%' AND `sem`='$sem'" ;
			
  			else if( $dept!='' && $sem == '')
				$sql = "SELECT `averify`,`susn`,`sfname`,`slname`,`exam_type`,`month`,`sem`,`challan_no`,`amount` FROM `exam_reg` WHERE `checks` ='$s'  AND `susn` LIKE '1RV__$dept%'" ;
			
			else if( $dept == '' && $sem == '')
				header('Location:examformad.php');
			
			else if( $dept == '' && $sem !='')
				$sql = "SELECT `averify`,`susn`,`sfname`,`slname`,`exam_type`,`month`,`sem`,`challan_no`,`amount` FROM `exam_reg` WHERE `checks` ='$s'  AND `susn` LIKE '1RV%' AND `sem` = '$sem'" ;
